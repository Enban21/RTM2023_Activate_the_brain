...
"""
 @file Base-value-judgmentTest.py
...
base-value-judgmenttest_spec = ["implementation_id", "Base-value-judgmentTest", 
         "type_name",         "Base-value-judgmentTest", 
...
# 
# </rtc-template>
class Base-value-judgmentTest(OpenRTM_aist.DataFlowComponentBase):
...
def RunTest():
    ...
    comp = manager.getComponent("Base-value-judgmentTest0")
    if comp is None:
        print('Component get failed.', file=sys.stderr)
        return False
    return comp.runTest()

def Base-value-judgmentTestInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=base-value-judgmenttest_spec)
    manager.registerFactory(profile,
                            Base-value-judgmentTest,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    Base-value-judgmentTestInit(manager)
    Base-value-judgment.Base-value-judgmentInit(manager)

    # Create a component
    comp = manager.createComponent("Base-value-judgmentTest")
